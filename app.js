...
function zeigeImageCropModal(id, imageDataUrl) {
  ...
  let cropSize = 350, cropX, cropY;
  let isDragging = false, dragStartX = 0, dragStartY = 0;
  let resizeMode = null; // null, 'nw', 'ne', 'sw', 'se', 'move'
  ...
  img.onload = function() {
    ...
    canvas.onwheel = (e) => {
      ...
      const newSize = Math.min(Math.max(100, cropSize + delta), Math.min(canvas.width - cropX, canvas.height - cropY));
      const diff = newSize - cropSize;
      cropX -= diff / 2;
      cropY -= diff / 2;
      cropSize = newSize;
